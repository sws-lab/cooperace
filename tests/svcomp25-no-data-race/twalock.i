...
extern twa_counter_t __twa_array[4096U];
...
twalock_acquire(twalock_t *l)
{
    ...
    vuint32_t dx = ((tx) - (vatomic32_read_acq(&l->grant)));
    if (dx == 0) {
        return;
    }
    if (dx > 1U) {
...
twalock_release(twalock_t *l)
{
    ...
    vatomic32_inc_rel(&__twa_array[((k + 1U) % 4096U)].val);
}
...
{
    vuint32_t at = ((t) % 4096U);
    vuint32_t u = vatomic32_read_acq(&__twa_array[at].val);
    vuint32_t k = vatomic32_read_rlx(&l->grant);
    vuint32_t dx = ((t) - (k));
    while (dx > 1U) {
        u = vatomic32_await_neq_acq(&__twa_array[at].val, u);
        dx = ((t) - (vatomic32_read_rlx(&l->grant)));
    }
}
...
check(void)
{
    ...
    do { if (!(g_cs_x == (3 + 0 + 0))) reach_error(); } while (0);
}
static void *
run(void *arg)
{
...
twa_counter_t __twa_array[4096U] ;;
twalock_t lock = { .ticket = { ._v = 0 }, .grant = { ._v = 0 } };
void
acquire(vuint32_t tid)
{
    if (tid == 3 - 1) {
        for (verification_loop_begin(); (verification_spin_start(), (!twalock_tryacquire(&lock)) ? 1 : (verification_spin_end(1), 0)); verification_spin_end(0)) {}
    } else {
