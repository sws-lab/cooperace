...
//
//     test/spinlock/rec_ticketlock.c
//
...
typedef struct rec_ticketlock_s { ticketlock_t lock; vatomic32_t owner; vuint32_t count; } rec_ticketlock_t; static inline void rec_ticketlock_init(rec_ticketlock_t *l) { ticketlock_init(&l->lock); vatomic32_init(&l->owner, (4294967295U)); l->count = 0; } static inline void rec_ticketlock_release(rec_ticketlock_t *l ) { if (l->count == 0) { vatomic32_write_rlx(&l->owner, (4294967295U)); (ticketlock_release(&l->lock )); } else { l->count--; } } static inline void rec_ticketlock_acquire(rec_ticketlock_t *l, vuint32_t id ) { do { if (!(id != (4294967295U) && "this value is reserved")) reach_error(); } while (0); if (vatomic32_read_rlx(&l->owner) == id) { l->count++; return; } (ticketlock_acquire(&l->lock )); vatomic32_write_rlx(&l->owner, id); } static inline vbool_t rec_ticketlock_tryacquire(rec_ticketlock_t *l, vuint32_t id ) { vuint32_t owner = vatomic32_read_rlx(&l->owner); do { if (!(id != (4294967295U) && "this value is reserved")) reach_error(); } while (0); if (owner == id) { l->count++; return 1; } if (owner != (4294967295U)) { return 0; } if (!((ticketlock_tryacquire)(&l->lock ))) { return 0; } vatomic32_write_rlx(&l->owner, id); return 1; }
...
rec_ticketlock_t lock = { .lock = { .next = { ._v = 0 }, .owner = { ._v = 0 } }, .owner = { ._v = (4294967295U) }, .count = 0, };
void
acquire(vuint32_t tid)
{
    if (tid == 3 - 1) {
        for (verification_loop_begin(); (verification_spin_start(), (!rec_ticketlock_tryacquire(&lock, tid)) ? 1 : (verification_spin_end(1), 0)); verification_spin_end(0)) {}
    } else {
