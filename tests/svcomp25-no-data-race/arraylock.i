...
{
    do { if (!(dividend)) reach_error(); } while (0);
    vsize_t result = ((divisor + (dividend - 1U)) / dividend) * dividend;
    do { if (!(result >= divisor)) reach_error(); } while (0);
    return result;
}
...
arraylock_init(arraylock_t *lock, arraylock_flag_t *flags, const vuint32_t len)
{
    ...
    do { if (!(len > 0)) reach_error(); } while (0);
    do { if (!((((len) != 0) && ((len) & ((len)-1)) == 0))) reach_error(); } while (0);
    lock->flags_len = len;
    lock->flags = flags;
...
arraylock_acquire(arraylock_t *lock, vuint32_t *slot)
{
    ...
    *slot = tail % lock->flags_len;
    for (verification_loop_begin(); (verification_spin_start(), (vatomic8_read_acq(&lock->flags[*slot].flag) == 0U) ? 1 : (verification_spin_end(1), 0)); verification_spin_end(0)) {}
}
static inline void
...
arraylock_flag_t flags[(3 + 1U)];
arraylock_t lock;
static __thread vuint32_t slot;
extern int (*__Static_assert_function (void)) [!!sizeof (struct { int __error_if_negative: ((3 <= (3 + 1U))) ? 2 : -1; })];
void
init(void)
{
    arraylock_init(&lock, flags, (3 + 1U));
}
void
