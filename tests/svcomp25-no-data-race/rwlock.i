...
{
    vatomic32_write_rlx(&l->wb, 0);
    semaphore_release(&l->rs, l->n);
}
static inline void
rwlock_read_acquire(rwlock_t *l)
{
    vatomic32_await_eq_rlx(&l->wb, 0);
    semaphore_acquire(&l->rs, 1U);
}
static inline vbool_t
rwlock_read_tryacquire(rwlock_t *l)
{
    if (vatomic32_read_rlx(&l->wb)) {
        return 0;
    }
    return semaphore_tryacquire(&l->rs, 1U);
}
static inline void
rwlock_read_release(rwlock_t *l)
{
    semaphore_release(&l->rs, 1U);
}
static inline vbool_t
...
reader_cs(vuint32_t tid)
{
    ...
    do { if (!(a && "g_cs_x and g_cs_y differ")) reach_error(); } while (0);
}
void
check(void)
{
    do { if (!(g_cs_x == g_cs_y)) reach_error(); } while (0);
    do { if (!(g_cs_x == 2)) reach_error(); } while (0);
}
static void *
writer(void *arg)
{
...
main(void)
{
    pthread_t t[(2 + 2)];
    init();
    verification_loop_bound(2 + 1);
    for (vuintptr_t i = 0; i < 2; i++) {
        (void)pthread_create(&t[i], 0, writer, (void *)i);
    }
    verification_loop_bound((2 + 2) + 1);
    for (vuintptr_t i = 2; i < (2 + 2); i++) {
...
rwlock_t lock = { .n = ((1U << 30U)), .wb = { ._v = 0 }, .rs = { .s = { ._v = ((1U << 30U)) } }, };
void
