...
void reach_error() { ((void) sizeof ((0) ? 1 : 0), __extension__ ({ if (0) ; else __assert_fail ("0", "safestack_relacy.c", 7, __extension__ __PRETTY_FUNCTION__); })); }
...
int Pop(void)
{
    while (__atomic_load_n(&stack.count, 5) > 1)
    {
        ...
        {
            int head2 = head1;
            if (__atomic_compare_exchange_n(&stack.head, &head2, next1, 0, 5, 5))
            {
...
void Push(int index)
{
    ...
    } while (!(__atomic_compare_exchange_n(&stack.head, &head1, index, 0, 5, 5)));
