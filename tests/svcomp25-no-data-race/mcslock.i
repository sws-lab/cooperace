...
mcslock_t lock = { .tail = { ._v = 0 } };
struct mcs_node_s nodes[3];
void
acquire(vuint32_t tid)
{
    if (tid == 3 - 1) {
        for (verification_loop_begin(); (verification_spin_start(), (!mcslock_tryacquire(&lock, &nodes[tid])) ? 1 : (verification_spin_end(1), 0)); verification_spin_end(0)) {}
    } else {
