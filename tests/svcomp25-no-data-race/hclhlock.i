...
typedef struct hclh_lock_s {
    vatomicptr_t local_queues[2];
    vatomicptr_t global_queue;
    hclh_qnode_t head;
} hclh_lock_t;
...
hclhlock_init(hclh_lock_t *hclh_lock)
{
    ...
    for (i = 0; i < 2; i++) {
        vatomicptr_write_rlx(&hclh_lock->local_queues[i], ((void *)0));
    }
}
static inline void
...
{
    vuint32_t state = 0;
    do { if (!(tnode)) reach_error(); } while (0);
    do { if (!(qnode)) reach_error(); } while (0);
    do { if (!(cluster < 2)) reach_error(); } while (0);
...
{
    vatomicptr_t *local_queue = ((void *)0);
    hclh_qnode_t *pred = ((void *)0);
    hclh_qnode_t *local_tail = ((void *)0);
    hclh_qnode_t *exp_tail = ((void *)0);
    vuint32_t state = 0;
    vuint32_t cluster = 0;
...
{
    vatomic32_t *state = ((void *)0);
    vuint32_t cur_state = 0;
    vuint32_t cluster = 0;
    do { if (!(tnode)) reach_error(); } while (0);
    do { if (!(tnode->qnode)) reach_error(); } while (0);
    state = &tnode->qnode->state;
    cur_state = vatomic32_read_rlx(state);
    cluster = ((cur_state)&0x3FFFFFFFU);
    ((cur_state) &= (~0x40000000U));
...
{
    vuint32_t cur_state = 0;
    vuint32_t exp_state = 0;
    vuint32_t new_state = 0;
    vatomic32_t *state = ((void *)0);
    do { if (!(tnode)) reach_error(); } while (0);
    do { if (!(tnode->pred)) reach_error(); } while (0);
    do { if (!(cluster < 0x3FFFFFFFU)) reach_error(); } while (0);
...
_hclh_await_grant_or_cluster_master(vatomic32_t *state, vuint32_t cluster)
{
    ...
    do { if (!(state)) reach_error(); } while (0);
    do { if (!(cluster < 0x3FFFFFFFU)) reach_error(); } while (0);
    required_state = _hclh_lock_get_default_state(cluster);
    ((required_state) &= (~0x40000000U));
    cur_state = vatomic32_read_acq(state);
    while (1) {
        if (cur_state == required_state) {
            return 1;
        }
        if (((cur_state)&0x3FFFFFFFU) != cluster || ((((cur_state)&0x80000000U) >> 31U) == 1U)) {
            return 0;
        }
...
_hclh_lock_get_default_state(vuint32_t cluster)
{
    vuint32_t state = 0;
    do { if (!(cluster < 0x3FFFFFFFU)) reach_error(); } while (0);
    ((state) = ((state) & (~0x3FFFFFFFU)) | (cluster));
    ((state) |= 0x40000000U);
    ((state) &= (~0x80000000U));
    return state;
}
...
hclh_lock_t lock;
hclh_tnode_t tnode[3];
hclh_qnode_t qnode[3];
void
init(void)
{
    hclhlock_init(&lock);
    for (vsize_t i = 0; i < 3; i++) {
        vuint32_t cluster = i % 2;
