...
caslock_t lock = { .lock = { ._v = 0 } };
void
acquire(vuint32_t tid)
{
    if (tid == 3 - 1) {
        for (verification_loop_begin(); (verification_spin_start(), (!caslock_tryacquire(&lock)) ? 1 : (verification_spin_end(1), 0)); verification_spin_end(0)) {}
    } else {
