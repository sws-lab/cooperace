...
{
    do { if (!(q && "q == NULL")) reach_error(); } while (0);
    do { if (!(v && "v == NULL")) reach_error(); } while (0);
...
run(void *arg)
{
    ...
    if (tid == 0) {
        for (vsize_t i = 0; i < 3; i++) {
            ...
            point->x = 1;
            point->y = 1;
            for (verification_loop_begin(); (verification_spin_start(), (bounded_spsc_enq(&g_queue, point) != QUEUE_BOUNDED_OK) ? 1 : (verification_spin_end(1), 0)); verification_spin_end(0))
                ;
        }
    } else {
        for (int i = 0; i < 3; i++) {
            void *r = ((void *)0);
            for (verification_loop_begin(); (verification_spin_start(), (bounded_spsc_deq(&g_queue, &r) != QUEUE_BOUNDED_OK) ? 1 : (verification_spin_end(1), 0)); verification_spin_end(0))
                ;
            point_t *point = (point_t *)r;
            if (point) {
                do { if (!(point->x == point->y)) reach_error(); } while (0);
                do { if (!(point->x == 1)) reach_error(); } while (0);
                free(point);
            } else {
                do { if (!(0 && "dequeued NULL")) reach_error(); } while (0);
            }
        }
    }
    return ((void *)0);
