                    ...
                    vuint32_t cores_per_node, vuint32_t core_id)
{
    ...
    for (i = 0, j = 0; i < locks_len; i++) {
        do { if (!(j < leaf_node_count && "you got over the boundary of leaf nodes")) reach_error(); } while (0);
        if (locks[i].is_leaf) {
...
{
    vsize_t level = 0;
    do { if (!(locks)) reach_error(); } while (0);
    do { if (!(locks_len > 0)) reach_error(); } while (0);
    do { if (!(num_levels > 0)) reach_error(); } while (0);
    vuint32_t total_nodes = 0;
    do { if (!(level_specs[0].num_nodes_per_parent == 1 && "top level should have one node")) reach_error(); } while (0);
    total_nodes = _hmcslock_count_nodes(level_specs, num_levels);
    do { if (!(locks_len >= total_nodes && "insufficient nodes, or too many nodes")) reach_error(); } while (0);
    _hmcslock_init_level(((void *)0), locks, &level, 0, level_specs, num_levels);
    do { if (!(level == locks_len && "recursion did not reach the expected depth")) reach_error(); } while (0);
}
static inline void
hmcslock_acquire(hmcslock_t *lock, hmcs_node_t *qnode, vsize_t num_levels)
{
    do { if (!(lock)) reach_error(); } while (0);
    do { if (!(qnode)) reach_error(); } while (0);
    do { if (!(num_levels > 0)) reach_error(); } while (0);
...
_hmcslock_release_helper(hmcslock_t *lock, hmcs_node_t *qnode, vuint64_t val)
{
    ...
    if (succ == ((void *)0)) {
        if (vatomicptr_cmpxchg(&lock->lock, qnode, ((void *)0)) == qnode) {
            return;
        }
        succ = (hmcs_node_t *)vatomicptr_await_neq_rlx(&qnode->next, ((void *)0));
    }
...
{
    hmcs_node_t *pred = ((void *)0);
    vatomic64_write_rlx(&qnode->status, 0x1UL);
    vatomicptr_write_rlx(&qnode->next, ((void *)0));
    pred = (hmcs_node_t *)vatomicptr_xchg(&lock->lock, qnode);
    if (pred) {
        vatomicptr_write_rel(&pred->next, qnode);
        vatomic64_await_neq_acq(&qnode->status, 0x1UL);
    } else {
        vatomic64_write_rlx(&qnode->status, 0x0UL);
    }
}
static inline void
_hmcslock_acquire_real(hmcslock_t *lock, hmcs_node_t *qnode, vsize_t depth)
{
    ...
    vatomic64_write_rlx(&qnode->status, (18446744073709551615ULL));
    vatomicptr_write_rlx(&qnode->next, ((void *)0));
    pred = (hmcs_node_t *)vatomicptr_xchg(&lock->lock, qnode);
    if (pred) {
        vatomicptr_write_rel(&pred->next, qnode);
        cur_status = vatomic64_await_neq_acq(&qnode->status, (18446744073709551615ULL));
        if (cur_status < ((18446744073709551615ULL) - 1)) {
            return;
        }
    }
    vatomic64_write_rlx(&qnode->status, 0x1UL);
    do { if (!(lock->parent && "parent is null")) reach_error(); } while (0);
    if (depth == 1) {
...
_hmcslock_release_real(hmcslock_t *lock, hmcs_node_t *qnode, vsize_t depth)
{
    ...
    hmcs_node_t *succ = ((void *)0);
    cur_count = vatomic64_read_rlx(&qnode->status);
    do { if (!(cur_count <= lock->threshold)) reach_error(); } while (0);
    if (cur_count < lock->threshold) {
    ...
    if (depth == 1) {
        _hmcslock_release_helper(lock->parent, &lock->qnode, 0x0UL);
    } else {
        _hmcslock_release_real(lock->parent, &lock->qnode, depth - 1);
    }
    _hmcslock_release_helper(lock, qnode, ((18446744073709551615ULL) - 1));
}
static void
                     ...
                     vsize_t num_levels)
{
    ...
    for (p = 0; p < level_specs[cur_level].num_nodes_per_parent; p++) {
        ...
        _hmcslock_init_level(cur_child, locks, idx, cur_level + 1, level_specs,
                             num_levels);
        vatomicptr_write(&cur_child->lock, ((void *)0));
    }
}
static vuint32_t
...
check(void)
{
    ...
    do { if (!(g_cs_x == (3 + 1 + 0))) reach_error(); } while (0);
}
static void *
run(void *arg)
{
    vuint32_t tid = (vuintptr_t)arg;
    verification_loop_bound((1 + 2));
    for (int i = 0; i == 0 || ((i) == 1 && ((tid) + 1 < (1) + 1)); i++) {
...
main(void)
{
    pthread_t t[3];
    init();
    verification_loop_bound(3 + 1);
    for (vuintptr_t i = 0; i < 3; i++) {
...
typedef struct {
    hmcslock_t hmcs_locks[((2 * 2 * 1) + (1 * 2) + (1))];
    hmcslock_t *leaf_locks[(3 + 1U)];
} lock_t;
typedef struct {
    hmcs_node_t qnode;
} ctx_t;
...
init(void)
{
    hmcslock_level_spec_t level_specs[3] = {
        {1, 1},
        {2, 1},
        {2, 1},
    };
    hmcslock_init(lock.hmcs_locks, ((2 * 2 * 1) + (1 * 2) + (1)), level_specs, 3);
    for (vuint32_t i = 0; i < (3 + 1U); i++) {
        lock.leaf_locks[i] = hmcslock_which_lock(
            lock.hmcs_locks, ((2 * 2 * 1) + (1 * 2) + (1)), level_specs, 3, 2, i);
    }
}
void
acquire(vuint32_t tid)
{
    ...
    hmcslock_acquire(lock.leaf_locks[coreid], &(ctx[tid].qnode), 3);
}
void
release(vuint32_t tid)
{
    const vuint32_t coreid = tid;
    hmcslock_release(lock.leaf_locks[coreid], &(ctx[tid].qnode), 3);
}
