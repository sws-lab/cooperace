...
{
    vatomicptr_write(&l->tail, ((void *)0));
}
static inline vbool_t
mcslock_tryacquire(mcslock_t *l, mcs_node_t *node)
{
    mcs_node_t *pred;
    vatomicptr_write_rlx(&node->next, ((void *)0));
    vatomic32_write_rlx(&node->locked, 1);
    pred = (mcs_node_t *)vatomicptr_cmpxchg(&l->tail, ((void *)0), node);
    return pred == ((void *)0);
}
static inline void
...
{
    mcs_node_t *next;
    if (vatomicptr_read_rlx(&node->next) == ((void *)0)) {
        next = (mcs_node_t *)vatomicptr_cmpxchg_rel(&l->tail, node, ((void *)0));
        if (next == node) {
            return;
        }
        vatomicptr_await_neq_rlx(&node->next, ((void *)0));
    }
...
{
    do { do { (void)(l); do { } while (0); } while (0); } while (0);
    return (vatomicptr_read_rlx(&node->next) != ((void *)0));
}
...
typedef struct rec_mcslock_s { mcslock_t lock; vatomic32_t owner; vuint32_t count; } rec_mcslock_t; static inline void rec_mcslock_init(rec_mcslock_t *l) { mcslock_init(&l->lock); vatomic32_init(&l->owner, (4294967295U)); l->count = 0; } static inline void rec_mcslock_release(rec_mcslock_t *l , mcs_node_t * ctx) { if (l->count == 0) { vatomic32_write_rlx(&l->owner, (4294967295U)); (mcslock_release(&l->lock , ctx)); } else { l->count--; } } static inline void rec_mcslock_acquire(rec_mcslock_t *l, vuint32_t id , mcs_node_t * ctx) { do { if (!(id != (4294967295U) && "this value is reserved")) reach_error(); } while (0); if (vatomic32_read_rlx(&l->owner) == id) { l->count++; return; } (mcslock_acquire(&l->lock , ctx)); vatomic32_write_rlx(&l->owner, id); } static inline vbool_t rec_mcslock_tryacquire(rec_mcslock_t *l, vuint32_t id , mcs_node_t * ctx) { vuint32_t owner = vatomic32_read_rlx(&l->owner); do { if (!(id != (4294967295U) && "this value is reserved")) reach_error(); } while (0); if (owner == id) { l->count++; return 1; } if (owner != (4294967295U)) { return 0; } if (!((mcslock_tryacquire)(&l->lock , ctx))) { return 0; } vatomic32_write_rlx(&l->owner, id); return 1; }
...
check(void)
{
    ...
    do { if (!(g_cs_x == (3 + 0 + 1))) reach_error(); } while (0);
}
static void *
run(void *arg)
{
    ...
    for (int i = 0; i == 0 || ((i) == 1 && ((tid) + 1 < (0) + 1)); i++) {
        ...
        for (int j = 0; j == 0 || ((j) == 1 && ((tid) + 1 < (1) + 1)); j++) {
            acquire(tid);
            cs();
        }
        verification_loop_bound((1 + 2));
...
rec_mcslock_t lock = { .lock = { .tail = { ._v = 0 } }, .owner = { ._v = (4294967295U) }, .count = 0, };
struct mcs_node_s nodes[3];
void
